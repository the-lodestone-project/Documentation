...
Lodestone uses [Shiki](https://shiki.matsu.io) to do syntax highlighting at build
...
your Lodestone configuration (`next.config.js` file). Once it's enabled globally,
...
Lodestone uses CSS variables to define the colors for tokens. You can inject a
...
Check out the
[code](https://github.com/shuding/Lodestone/blob/main/docs/pages/docs/guide/syntax-highlighting.mdx)
to see how it works.
...
Lodestone configuration (`next.config.js` file).
...
`mdxOptions.rehypePrettyCodeOptions` option in your Lodestone config inside
`next.config.js`:
...
Lodestone currently doesn't support specifying multiple themes. Because Shiki

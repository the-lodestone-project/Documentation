...
In Lodestone, the site and page structure can be configured via the co-located
...
import { Callout, FileTree } from 'Lodestone/components'
import menuImage from '../../../public/assets/docs/menu.png'

<Callout>
  Read more about Lodestone's `_meta.json` files
...
Then Lodestone knows that the `fruits` key in `_meta.json` defines a folder with an
...
```json filename="pages/_meta.json" {6-9}
{
  ...
  "github_link": {
    "title": "Lodestone",
    "href": "https://github.com/shuding/Lodestone"
  }
}
```
...
<div className="mt-4 text-center text-sm">
  ...
  ↗](https://stackblitz.com/edit/Lodestone-2-docs-eszspq?file=pages%2F_meta.json)
</div>
...
<div className="mt-8 text-center text-sm">
  [Live example on StackBlitz
  ↗](https://stackblitz.com/edit/Lodestone-2-docs-2qopvp?file=pages%2F_meta.json)
</div>
...
By default, Lodestone renders the MDX content (such as `h1`, `h2`, `h3` etc.) with
themed components, inside a content container. You can use the `"raw"` layout to
let Lodestone to not inject any styles to the content:
